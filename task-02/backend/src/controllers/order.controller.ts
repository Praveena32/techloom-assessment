...
export class OrderController {
    ...
    try {
      ...
      if (result.isExisting) {
        return res.status(200).json({
          success: true,
          isDuplicate: true,
          message: 'Retrieved order via idempotency key',
          data: result.order,
        });
      }
  ...
  static async cancelOrRefund(req: Request, res: Response) {
    try {
      const id = req.params.id as string;
      const { reason } = req.body;
      const result = await OrderService.cancelOrRefundOrder(id, reason);
      res.json({
        success: true,
